...
void program(void)
{
	...
	while(1){
		...
		while(htim1_flg == 0);
		htim1_flg = 0;
		//HAL_GPIO_TogglePin(TEST02_GPIO_Port,TEST02_Pin);
...
void init_drive()
{
	...
	drive.left_duty_bk = 18;
	drive.left_value[0] = 0;
	drive.left_value[1] = 0;
	...
	drive.left_duty_bk1 = 18;
	drive.left_value1[0] = 0;
	drive.left_value1[1] = 0;
	...
	drive.right_duty_bk = 18;
	drive.right_value[0] = 0;
	drive.right_value[1] = 0;

	drive.right_on1 = 0;
	drive.right_on_bk1 = 1;
	//drive.right_duty1 = htim3.Init.Period / 2;
	//drive.right_duty1 = 30;
	drive.right_duty1 = 18;
	//drive.right_duty_bk1 = htim3.Init.Period / 2;
	//drive.right_duty_bk1 = 30;
	drive.right_duty_bk1 = 18;
	drive.right_value1[0] = 0;
	drive.right_value1[1] = 0;
...
unsigned char read_fact()
{
	...
	flg = 1;
	flash_read_page(&tmp.header[0],FLASH_FACT_BANK2_SECTOR);
...
void write_fact()
{
	...
	flash_write_page(&fact.header[0],FLASH_FACT_BANK2_SECTOR);
}
...
void flash_read_page(unsigned char *d_pos,unsigned char sector)
{
	...
	for(i = 0;i < sizeof(tmp);i += 4){
		*(s_pos + j++) = Flash_Read((0x08100000 | (sector << 1) << 16) + i);
	}
	__nop();
...
void rgb_pwm()
{
	...
	if(rgb_pwm_tot){
		...
		if(rgb_chk_ch3_flg){
			...
			offset = (fact.ch3.offset[0] << 8) | fact.ch3.offset[1];
			...
			if(flg){
				...
				//Check RGB LEFT & RIGH space level 1
				if(fact.ch3.thr_ok_value){
					//HAL_GPIO_WritePin(LED_TEST1_GPIO_Port,LED_TEST1_Pin,0);
				}
				else{
		...
		if(rgb_chk_ch2_flg){
			channel = RGB_CH2;
			offset = (fact.ch2.offset[0] << 8) | fact.ch2.offset[1];
			flg = 1;
		...
		if(rgb_chk_ch4_flg){
			channel = RGB_CH4;
			offset = (fact.ch4.offset[0] << 8) | fact.ch4.offset[1];
			flg = 1;
		...
		if(rgb_chk_ch1_flg){
			channel = RGB_CH1;
			offset = (fact.ch1.offset[0] << 8) | fact.ch1.offset[1];
			flg = 1;
		...
		if(rgb_chk_ch5_flg){
			channel = RGB_CH5;
			offset = (fact.ch5.offset[0] << 8) | fact.ch5.offset[1];
			flg = 1;
		...
		if(rgb_chk_ch3_flg){
			...
			if(drive.duty_limit < drive.step_duty){
				drive.duty_limit = drive.step_duty;
			}
			
			#if 1///tien
			drive.left_on1 = 1;
			//drive.left_duty1 = 30;
			if(drive.left_duty1 < drive.duty_set){
				drive.left_duty1++;
			}
			else{
				drive.left_duty1 = drive.right_duty;
		    }
			#endif
			#if 0//lui
			drive.left_on = 1;
			//drive.left_duty1 = 30;
			if(drive.left_duty < drive.duty_set){
				drive.left_duty++;
			}
			#endif
			#if 0//lui
			drive.right_on1 = 1;
			//drive.right_duty1 = 30;
			if(drive.right_duty1 < drive.duty_set){
				drive.right_duty1++;
			}
			#endif
			#if 1//tien
			drive.right_on = 1;
			//drive.right_duty1 = 30;
			if(drive.right_duty < drive.duty_set){
				drive.right_duty++;
			}
			else{
				drive.right_duty = drive.right_duty;
			}
			#endif
		}
			...
			drive.left_on1 = 1;
			if(drive.left_duty1 > drive.duty_limit){
				drive.left_duty1 = drive.left_duty1 - drive.step_duty;
		    }
			else{
				drive.left_duty1 = drive.duty_limit;
			}
			#if 0
			drive.right_on = 1;
			if(drive.right_duty > drive.duty_limit){
				drive.right_duty = drive.right_duty - drive.step_duty;
		}
			#endif // 0
			drive.right_on = 1;
			//drive.left_duty1 = 30;
			if(drive.right_duty < drive.duty_set){
				...
				drive.right_duty = drive.duty_set;
			}
			...
			drive.left_on1 = 1;
			//drive.left_duty1 = 30;
			if(drive.left_on1 < drive.duty_set){
				drive.left_on1++;
		}
			...
			if(drive.left_duty1 < drive.duty_set){
				...
				drive.left_duty1 = drive.duty_set;
			}

			drive.right_on1 = 0;
			//drive.left_duty1 = 30;
			if(drive.right_duty1 > 0){
				if(drive.right_duty1 > drive.duty_limit){
					drive.right_duty1 = drive.right_duty1 - drive.step_duty;
				}
				else{
					drive.right_duty1 = 0;
				}
...
void pwm_drive_opera(void)
{
	...
	if(drive.left_on){
		if(drive.left_on ^ drive.left_on_bk){
			if(drive.left_duty){
				drive.left_on_bk = drive.left_on;
				drive.left_duty_bk = drive.left_duty;
				...
				duty = drive.left_duty;
				set_duty(&htim3,duty,TIM_CHANNEL_1);
				HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_1);			
			}
		}
		else if(drive.left_duty ^ drive.left_duty_bk){
			drive.left_duty_bk = drive.left_duty;
			duty = drive.left_duty;
			set_duty(&htim3,duty,TIM_CHANNEL_1);
		}
	...
	//WHEEL LEFT1
	if(drive.left_on1){
		if(drive.left_on1 ^ drive.left_on_bk1){
			if(drive.left_duty1){
				drive.left_on_bk1 = drive.left_on1;
				drive.left_duty_bk1 = drive.left_duty1;
				...
				duty = drive.left_duty1;
				set_duty(&htim3,duty,TIM_CHANNEL_2);
				HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_2);
			}
		}
		else if(drive.left_duty1 ^ drive.left_duty_bk1){
			drive.left_duty_bk1 = drive.left_duty1;
			duty = drive.left_duty1;
			set_duty(&htim3,duty,TIM_CHANNEL_2);
		}
	...
	//WHEEL RIGHT
	if(drive.right_on){
		if(drive.right_on ^ drive.right_on_bk){
			if(drive.right_duty){
				drive.right_on_bk = drive.right_on;
				drive.right_duty_bk = drive.right_duty;
				...
				duty = drive.right_duty;
				set_duty(&htim3,duty,TIM_CHANNEL_3);
				HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_3);			
			}
		}
		else if(drive.right_duty ^ drive.right_duty_bk){
			drive.right_duty_bk = drive.right_duty;
			duty = drive.right_duty;
			set_duty(&htim3,duty,TIM_CHANNEL_3);
		}
	...
	//WHEEL RIGHT
	if(drive.right_on1){
		if(drive.right_on1 ^ drive.right_on_bk1){
			if(drive.right_duty1){
				drive.right_on_bk1 = drive.right_on1;
				drive.right_duty_bk1 = drive.right_duty1;
				...
				duty = drive.right_duty1;
				set_duty(&htim3,duty,TIM_CHANNEL_4);
				HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_4);
			}
		}
		else if(drive.right_duty1 ^ drive.right_duty_bk1){
			drive.right_duty_bk1 = drive.right_duty1;
	...
	if((drive.left_on == 0) && (drive.left_on1 == 0) && (drive.right_on == 0) && (drive.right_on1 == 0)){
		//disable PWM ENA
...
//uart2 send string
void uart2_send_string(char *s_pos,unsigned short len)
{
	unsigned char* d_pos;
	unsigned short i;
	d_pos = &t2_buff[0];
